class PostService {
  ...
  async post(postId) {
    const result = await this._pool.query(
      `SELECT posts.id AS id, user_id, users.nickname AS nickname, users.img_path AS user_img, title, title_content, content, posts.img_path AS post_img, posts.create_at AS create_at 
      FROM posts LEFT JOIN users 
      ON posts.user_id = users.id 
      WHERE posts.id = $1`,
      [postId],
  ...
  //포스트 수정시 대표이미지 삭제
  async deletePostImage(postId) {
    const result = await this._pool.query(`UPDATE posts SET img_path = '' WHERE id = $1`, [postId]);
    return result.rows[0];
  }
  //포스트 삭제
